...
これはGitHub/Linearへ起票可能な作業定義です。実際のIssue・ブランチ・PRは作成していません。番号は識別子であり、実装順序ではありません。各作業の「対象」は基盤・技術検証として関与する要件を含む**関連要件**です。要件の実装責任を持つタスクと、成立可否を左右する技術検証タスクは[追跡表](20_TRACEABILITY.md)を正とします。各作業は[タスク雛形](../templates/IMPLEMENTATION_TASK.md)を使い、受入条件と実施した試験を記録します。
