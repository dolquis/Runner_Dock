...
主なcodeは`AUTH_EXPIRED`、`PERMISSION_OR_POLICY`、`RATE_LIMITED`、`RUNNER_BUSY`、`STATUS_STALE`、`PATH_NOT_OWNED`、`PROTOCOL_MISMATCH`、`REVISION_CONFLICT`、`REQUIRES_CONFIRMATION`、`CHECKSUM_MISMATCH`、`METHOD_NOT_SERVED`です。`METHOD_NOT_SERVED`は、メソッド一覧にはあるがそのAgent版が処理を提供していない場合に返し、成功や空の結果へ丸めません。生HTTP応答やOSコマンドラインをそのままdetailsに入れません。
