...
このパッケージの`README.md`、`AGENTS.md`、`docs/`、`templates/`を初回コミットに入れます。製品名はRunner Dockです。内部識別子は`runnerdock`を決定案とします([ADR-015](docs/17_ADR.md))。商標・既存名との重複確認は製品の配布開始前の確認事項として残ります。文書のみを置いた状態でリリースタグを作成しません。
