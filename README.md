# LocalForge 開発開始ドキュメント一式

**仮称:** LocalForge / **版:** 0.1.0-design / **作成・仕様確認日:** 2026-09-19

Windows PC 1台を、WindowsネイティブとWSL2 UbuntuのGitHub Actions実行環境として構築・運用するための、**新規独自開発プロジェクトの設計基準**です。HomeRunのフォークやコード移植を前提にしません。

> このパッケージは開発用文書です。アプリ、インストーラー、動作確認済みの実装は含みません。採用する設計、確認できた外部仕様、実機検証が必要な仮説を区別しています。製品名・公開ライセンス・公開先は未確定です。
